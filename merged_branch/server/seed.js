...
// Drops and re-creates the nurses and tasks collections with data
...
//pre-populate the rooms
const rooms = [];
for (const floor of ["1", "2", "3", "4"]) {
  for (let i = 1; i <= 15; i++) {
    rooms.push({
      roomNumber: `${floor}${String(i).padStart(2, "0")}`,  //for rooms: 101-115, 201-215, 301-315, 401-415
      floor,
      status: "empty",
...
async function seed() {
  ...
  try {
    ...
    const nResult = await db.collection("nurses").insertMany(nurses);
    console.log(`Inserted ${nResult.insertedCount} nurses`);
    ...
    const rResult = await db.collection("rooms").insertMany(rooms);
    await db.collection("rooms").createIndex({ roomNumber: 1 }, { unique: true });
    console.log(`Inserted ${rResult.insertedCount} rooms`);
